...
namespace Nop.Services.Localization
{
    public static class LocalizationExtensions
    {
        ...
        /// <param name="returnDefaultValue">A value indicating whether to return default value (if localized is not found)</param>
        /// <returns>Localized value</returns>
        public static string GetLocalizedFriendlyInfo<T>(this T plugin, ILocalizationService localizationService,
            int languageId, bool returnDefaultValue = true)
            where T : IPlugin
        {
            if (localizationService == null)
            ...
            string systemName = plugin.PluginDescriptor.SystemName;
            //localized value
            string resourceName = string.Format("Plugins.FriendlyInfo.{0}",
                systemName);
            string result = localizationService.GetResource(resourceName, languageId, false, "", true);
